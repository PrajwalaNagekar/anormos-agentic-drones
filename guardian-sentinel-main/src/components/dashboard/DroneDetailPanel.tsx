...
import { X, Battery, Signal, MapPin, Gauge, Compass, Plane, Radio, Sun } from 'lucide-react';
...
const DroneDetailPanel = ({ drone: d, onClose }: { drone: Drone; onClose: () => void }) => {
  ...
  const TelemetryRow = ({ icon: Icon, label, value, unit }: { icon: any; label: string; value: any; unit?: string }) => (
    <div className="flex items-center justify-between py-1.5 border-b border-border/50">
      <span className="flex items-center gap-2 text-[10px] font-mono text-muted-foreground uppercase"><Icon className="w-3 h-3" /> {label}</span>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-border flex items-center justify-between">
        ...
        <button onClick={onClose} className="p-1 hover:bg-secondary rounded-sm"><X className="w-4 h-4 text-muted-foreground" /></button>
      </div>
        ...
        <div className="p-3 space-y-4">
            ...
            <div className="mt-3 space-y-2">
              <div>
                <div className="flex justify-between text-[10px] font-mono">
                  ...
                  <span className={d.battery_pct < 30 ? 'text-destructive' : 'text-primary'}>{d.battery_pct}%</span>
                </div>
                <Progress value={d.battery_pct} className="h-1.5 mt-1" />
              </div>
              {d.solar_active && (
                <div className="flex items-center gap-2 text-[10px] font-mono text-accent">
                  <Sun className="w-3 h-3" /> Solar Active: +{d.solar_charge_rate}W
                </div>
              )}
              <div>
                <div className="flex justify-between text-[10px] font-mono">
                  <span className="text-muted-foreground">Battery Health</span>
                  <span className="text-foreground">{d.battery_health}%</span>
                </div>
                <Progress value={d.battery_health} className="h-1.5 mt-1" />
              </div>
              <div className="flex justify-between text-[10px] font-mono">
                <span className="text-muted-foreground">Est. Flight Time</span>
                <span className="text-foreground">{d.estimated_flight_time} min</span>
              </div>
              <div>
                <div className="flex justify-between text-[10px] font-mono">
          ...
          <div className="military-panel p-3">
            ...
            <div className="flex flex-wrap gap-1">
              {d.payload.map((p, i) => (
                <span key={i} className="text-[10px] font-mono px-2 py-0.5 bg-secondary rounded-sm text-secondary-foreground">{p}</span>
              ))}
            </div>
          </div>
          ...
          <div className="military-panel p-3">
            ...
            <div className="space-y-1 font-mono text-[10px]">
              ...
              <p className={`pl-9 ${d.battery_pct > 20 ? 'text-primary' : 'text-destructive'}`}>
                └─ {d.battery_pct > 20 ? '✓ PASS' : '✗ FAIL — RTB Triggered'}
              </p>
