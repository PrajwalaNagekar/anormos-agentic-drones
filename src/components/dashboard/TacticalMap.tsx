...
const TacticalMap = ({ drones, selectedDrone, onSelectDrone }: Props) => {
  ...
  // Map Qatar area lat/lng to canvas coords
    ...
    <div className="relative w-full h-full bg-background grid-tactical overflow-hidden">
      ...
      <div className="absolute top-3 left-3 z-20">
        <div className="military-panel px-3 py-1.5">
          <span className="text-[10px] font-mono text-primary uppercase">TACTICAL MAP — QATAR AO</span>
        </div>
      </div>
          ...
          >
            ...
            {['active', 'pursuit'].includes(d.status) && (
              <span
                ...
                style={{ backgroundColor: `${color}33`, width: 24, height: 24, left: -4, top: -4 }}
              />
            )}
            {/* Drone dot */}
            <span
            ...
            {/* Label */}
            <span className="absolute top-5 left-1/2 -translate-x-1/2 whitespace-nowrap text-[9px] font-mono opacity-0 group-hover:opacity-100 transition-opacity px-1 py-0.5 bg-card border border-border rounded-sm"
              style={{ color }}>
              {d.callsign}
            </span>
          </button>
